...
def associate_by_id(
    ...
    for mid, m in enumerate(masks):
        ...
        if len(valid_depths) > 0:
            ...
            if use_percentile and len(valid_depths) >= 4:
                ...
                if k_factor * depth_std < min_abs_error:
                    ...
                    method_name = "绝对误差方法"    
            # 确保深度范围在合理范围内
            depth_min = max(depth_min, depth_mean - 0.2)
            depth_max = min(depth_max, depth_mean + 0.2)
        ...
        if oid is not None:
            ...
            new_obj = SceneObject(pose=Tw.copy(), id=external_id, initial_label=label, voxel_size=voxel_size)
