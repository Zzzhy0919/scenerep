...
def load_config(config_path="data_demo_config.yaml"):
    ...
    try:
        ...
        # # 验证必需的配置项
        # required_keys = ['dataset', 'state_ranges', 'mask', 'camera', 'tsdf', 'visualization']
        ...
        # if 'method' not in mask_config:
        #     raise ValueError("mask配置缺少method字段")
        
        # method = mask_config['method']
        # if method == "color":
        #     if 'color_map' not in mask_config or 'tolerance' not in mask_config:
        #         raise ValueError("color模式需要color_map和tolerance配置")
        # elif method == "json":
        #     if 'detection_dir' not in mask_config or 'score_threshold' not in mask_config:
        #         raise ValueError("json模式需要detection_dir和score_threshold配置")
        # else:
        #     raise ValueError(f"不支持的mask提取方法: {method}")
...
# # ----------------- 从配置文件加载参数 -----------------
# # 状态区间定义（有序列表，可重复）
# state_ranges_config = config['state_ranges']
# state_ranges = []
# for item in state_ranges_config:
#     start, end, state = item
#     # 将null转换为None
#     # obj_id = None if obj_id is None else obj_id
#     state_ranges.append((start, end, state))
...
if mask_method == "color":
    ...
    detection_dir = mask_config['detection_dir']
    hungarian_dir = mask_config['hungarian_dir']
...
# def get_object_id_in_ee(idx, state_ranges, masks, end_effector_mask, relations, T_lfw, T_rfw, objects):
#     if not hasattr(get_robot_state, "last_state"):
#         get_robot_state.last_state = "idle"
#         get_robot_state.last_obj_id_in_ee = None
#     current_state = "idle"
#     preset_obj_id = None
...
#             current_state = state
#             break
#     if current_state == "grasping" and get_robot_state.last_state == "idle":
#         # if masks and end_effector_mask is not None and np.sum(end_effector_mask) > 0:
#         #     max_iou = 0
#         #     obj_id_in_ee = None
            
#         #     for mask_info in masks:
#         #         mask = mask_info.get("mask")
#         #         obj_id = mask_info.get("id")
                
#         #         intersection = np.logical_and(mask, end_effector_mask).sum()
#         #         union = np.logical_or(mask, end_effector_mask).sum()
#         #         iou = intersection / union if union > 0 else 0
                
#         #         # 检查该物体是否有in或on关系
#         #         has_constraint = False
#         #         if relations is not None and obj_id in relations:
#         #             in_relations = relations[obj_id].get("in", [])
#         #             on_relations = relations[obj_id].get("on", [])
                    
#         #             if in_relations or on_relations:
#         #                 has_constraint = True
                
#         #         if iou > max_iou and not has_constraint:
#         #             max_iou = iou
#         #             obj_id_in_ee = obj_id
            
#         #     # 如果找不到合适的物体，则使用预设值
#         #     if obj_id_in_ee is None:
#         #         obj_id_in_ee = preset_obj_id
#         gripper_center_w = np.zeros(3)
#         gripper_center_w[0] = (T_lfw[0, 3] + T_rfw[0, 3]) / 2.0
#         gripper_center_w[1] = (T_lfw[1, 3] + T_rfw[1, 3]) / 2.0
#         gripper_center_w[2] = (T_lfw[2, 3] + T_rfw[2, 3]) / 2.0

#         min_distance = float('inf')
#         for obj in objects:
#             if obj is None or not hasattr(obj, 'pose_cur'):
#                 continue

#             obj_center = obj.pose_cur[:3, 3]
            
#             # 计算物体中心与夹爪中点的距离
#             distance = np.linalg.norm(obj_center - gripper_center_w)

#             has_constraint = False
#             if relations is not None and obj.id in relations:
#                 in_relations = relations[obj.id].get("in", [])
#                 on_relations = relations[obj.id].get("on", [])
                
#                 if in_relations or on_relations:
#                     has_constraint = True
            
#             if distance < min_distance and not has_constraint:
#                 min_distance = distance
#                 obj_id_in_ee = obj.id

#     elif current_state == "idle" and get_robot_state.last_state == "releasing":
#         obj_id_in_ee = None
#     else:
#         obj_id_in_ee = get_robot_state.last_obj_id_in_ee
    
#     # 更新last_state和last_obj_id_in_ee
#     get_robot_state.last_state = current_state
#     get_robot_state.last_obj_id_in_ee = obj_id_in_ee
...
def main():
    ...
    # print(f"状态区间数量: {len(state_ranges)}")
    print(f"掩码提取方法: {mask_method}")
    if mask_method == "color":
        print(f"掩码容差: {mask_tol}")
        print(f"颜色映射: {list(color_map.keys())}")
    elif mask_method == "json":
        print(f"检测目录: {detection_dir}")
        print(f"置信度阈值: {score_threshold}")
    print(f"清除底部比例: {config['mask'].get('clear_bottom_ratio', 0.1)}")
    print(f"Mask缩小: {'启用' if config['mask'].get('shrink_mask', True) else '禁用'}")
    if config['mask'].get('shrink_mask', True):
        print(f"  腐蚀核大小: {config['mask'].get('shrink_kernel_size', 5)}")
    print(f"背景TSDF体素大小: {config['tsdf']['bg_voxel_size']}")
    ...
    for idx in tqdm(range(len(rgb_files)), desc="Processing dataset"):
        ...
        if pose_ref_cfg.get('enabled', False) and objects and not skip_fusion:
            ...
            if pose_ok:
                ...
                print(T_offset)
            # else:
        # if T_offset is not None and not pose_ok:
        #     print(T_offset)
        #     T_cw = T_offset @ T_cw
        ...
        # # 处理masks：缩小mask以避免桌面噪声
        #         # 1. 设置box范围和采样步长（从配置文件读取）
        # hand_config = config['hand_projection']
        # xmin, xmax, xstep = hand_config['xmin'], hand_config['xmax'], hand_config['xstep']
        # ymin, ymax, ystep = hand_config['ymin'], hand_config['ymax'], hand_config['ystep']
        # zmin, zmax, zstep = hand_config['zmin'], hand_config['zmax'], hand_config['zstep']
        ...
        # update the reappeared object pose with icp
        # for m in masks:
        #     if m.get("id") == obj_id_in_ee:
        #         continue
        #     obj = find_object_by_id(m.get("id"), objects)
        #     if obj is not None and obj.last_update_frame < processed_frames - 1:
        #         print(f"reappeared object {obj.id}")
        #         print(f"last_update_frame: {obj.last_update_frame}")
        #         print(f"processed_frames: {processed_frames}")
        #         icp_reappear(obj, T_cw, K, m["mask"], rgb, depth)
        #     if obj is not None:
        #         obj.last_update_frame = processed_frames
            ...
            if show_bounding_boxes:
                ...
                if len(V) > 0:
                    ...
                    if show_object_poses:
                        # pose_key = f"pose{i}"
                        # if pose_key in mesh_nodes:
                        #     try: 
                        #         # 移除旧的位姿节点
                        #         for node in mesh_nodes[pose_key]:
        ...
        # # 每处理50帧可视化一次obj.points
        # if (idx % 500 == 0 or idx == len(rgb_files) - 1) and objects:
        #     import open3d as o3d
        #     obj = objects[0]  # 只可视化第一个对象，如需全部可遍历
        #     pcd = o3d.geometry.PointCloud()
        #     pcd.points = o3d.utility.Vector3dVector(obj.points)
        #     if hasattr(obj, 'colors') and obj.colors is not None and len(obj.colors) == len(obj.points):
        #         c = obj.colors.copy()
        #         if c.max() > 1.5:
        #             c = c / 255.0
        ...
        #     # 坐标系
        #     axis_world = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.3, origin=[0,0,0])
        #     axis_pose = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.06)
        #     axis_pose.transform(obj.pose)
        #     axis_current = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)
        #     print(obj.T)
        #     axis_current.transform(obj.pose @ obj.T)

        #     o3d.visualization.draw_geometries([pcd, axis_world, axis_pose, axis_current],
        #                                       window_name=f"Obj Points Frame {idx}")
