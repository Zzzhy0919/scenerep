...
DATASET_DIR = "/media/zhy/bcd58cff-609f-4e23-89f6-9fc2e8b36fea/datasets/apple_2"  # 请替换为实际的数据集路径
EVAL_FILE = os.path.join(DATASET_DIR, "eval", "object_1.txt") # 请替换为实际的评估文件名
...
CSV_FILE = os.path.join(DATASET_PATH, "Take 2025-09-23 06.01.55 PM.csv")  # 请替换为实际的CSV文件名
DETECTION_DIR = os.path.join(DATASET_DIR, "detection_h")
OBJECT_NAME = "apple" # 请替换为实际的物体名称
OBJECT_ID = 1 # 请替换为实际的物体ID
...
class PoseEvaluator:
    ...
    #         # 解析姿态数据: 帧号 秒 纳秒 x y z qx qy qz qw
    #         parts = line.split()
    #         if len(parts) != 10:
    #             print(f"警告: 行格式错误 '{line}'")
    #             continue
    ...
    #         quaternion = np.array([float(parts[6]), float(parts[7]), float(parts[8]), float(parts[9])])
    ...
    #     return estimated_poses, segments
